...
/* Optically center a glyph on the 32×32 grid. Glyphs (Material Symbols) are drawn to their
   own bounds and don't all sit centered — some land dead-on, others off by ~1px, each
   differently — so a single nudge can't fix them. Measure the path box and translate its
   center to (16,16). getBBox ignores the element's own transform, so re-measuring is stable;
   under a quarter-px we leave it (an already-centered glyph gets no transform).
   Mirrors the DS ProductTile's CenteredGlyph (vipre-design-system). */
...
export function ProductTile({ glyph, muted, size = 32 }) {
  ...
  if (muted) {
    return (
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none" style={{ display: 'block' }} aria-hidden="true">
        ...
        <CenteredGlyph glyph={glyph} style={{ fill: 'var(--vds-midnight-400)' }} />
      </svg>
    )
  }
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none" style={{ display: 'block' }} aria-hidden="true">
      ...
      <CenteredGlyph glyph={glyph} fill={`url(#${gl})`} />
      <defs>
...
/* Nav "section" tiles (Dashboard / Customers / Overview). The tile stays a neutral dark
   square; only the GLYPH color changes with state, mirroring the product sub-item rows:
   muted (midnight-400) at rest, white when the row is selected. Selection is carried by the
   ROW's cobalt fill — a cobalt tile on that cobalt row would vanish, which is what "looked
   off". Solid glyph, never a thin outline, so it stays legible on the collapsed rail.
   `selected` is opt-in (v2 only): left undefined, the tile keeps its original #E3E3E3 glyph,
   so the frozen v1 / Symphony shells are untouched. */
const TILE_BG = 'var(--nav-tile-bg, var(--vds-midnight-800))'
const navGlyph = (selected) =>
  selected === undefined ? '#E3E3E3' : selected ? '#fff' : 'var(--vds-midnight-400)'

export function OverviewTile({ size = 32, className, style, selected }) {
  return (
    <svg width={size} height={size} viewBox="1 0.5 32 32" fill="none" className={className} style={{ display: 'block', ...style }} aria-hidden="true">
      <path d="M1 8.5C1 4.08172 4.58172 0.5 9 0.5H25C29.4183 0.5 33 4.08172 33 8.5V24.5C33 28.9183 29.4183 32.5 25 32.5H9C4.58172 32.5 1 28.9183 1 24.5V8.5Z" style={{ fill: TILE_BG }} />
      <path d="M15.55 22.7L20.725 16.5H16.725L17.45 10.825L12.825 17.5H16.3L15.55 22.7ZM13.577 25.9422L14.577 19H9.952L18.1922 7.10575H19.423L18.4327 15H23.9325L14.8077 25.9422H13.577Z" fill={navGlyph(selected)} />
    </svg>
  )
}
// `outline` renders the 4 panels as strokes instead of solid fills (used by the MSP shell).
export function DashboardTile({ size = 32, className, style, outline = false, selected }) {
  // v2 (selected passed): solid panels in the state glyph color. v1 (selected undefined):
  // the original outline-or-solid grey.
  const blockProps = selected !== undefined
    ? { fill: navGlyph(selected) }
    : outline
    ? { fill: 'none', stroke: '#E3E3E3', strokeWidth: 1.5 }
    : { fill: '#E3E3E3' }
  return (
    <svg width={size} height={size} viewBox="1 0.5 32 32" fill="none" className={className} style={{ display: 'block', ...style }} aria-hidden="true">
      <rect x="1" y="0.5" width="32" height="32" rx="8" style={{ fill: TILE_BG }} />
